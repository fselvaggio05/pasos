		...
		<div class="row mt-1">
			...
			<div class="col-9">
				<h4 class="text-center my-4 mb-5 text-decoration-underline fw-bold ">Listado
					de practicas discapacidad realizadas</h4>
				<div class="container">
				<form action="listadodiscapacidad" method="post">
					...
					<table class="table table-striped my-2" id="tablaPrescripciones">
						<thead>
							<tr>
								...
								<th scope="col">Monto practica</th>
							</tr>
						</thead>
						<tbody>
							<c:if test="${turnos!=null}">
								<c:forEach var="tur" items="${turnos}">
									<tr>
										<td><c:out
												   value="${tur.getPaciente().getObra_social().getId_obra_social()} - ${tur.getPaciente().getObra_social().getNombre_os()}"></c:out>
										</td>
										<td><c:out value="${tur.getHorario().getPractica().getId_practica()} - ${tur.getHorario().getPractica().getDescripcion()}"></c:out>
										</td>
										<td><c:out value="${tur.getFecha_t()}"></c:out>
										</td>
										<td><c:out
												   value="${tur.getPaciente().getApellido()} , ${tur.getPaciente().getNombre()}"></c:out>
									  <td>
                                          <c:forEach var="monto" items="${tur.getHorario().getPractica().getMontos()}">
                                                <c:out value="${monto.getMonto()}" />
                                              <br/>
                                          </c:forEach>
                        </td>
                    </tr>
