		...
		<div class="row mt-1">
			...
			<div class="col-9">
				...
				<div class="container ">
					...
					<h5 class="small fst-italic text-right mt-3">
						Bienvenido
						<c:out value="${usuario.apellido},${usuario.nombre}"></c:out>
					</h5>

					<h4
						class="text-center my-4 mb-5 text-decoration-underline fw-bold ">Turnos
						registrados</h4>
						...
						<tbody>
							...
							<c:if test="${turnosPaciente!=null}">
								...
								<c:forEach var="tur" items="${turnosPaciente}">
									...
									<tr>
										<td><c:out
												value="${tur.getHorario().getProfesional().getApellido()} , ${tur.getHorario().getProfesional().getNombre()}"></c:out>
										</td>

										<td><c:out
												value="${tur.getHorario().getPractica().getDescripcion()}"></c:out>
										</td>

										<td><c:out value="${tur.getFecha_t()} ${tur.getHora_t()}"></c:out>
										</td>
										<td><c:out
												value="${tur.getPaciente().getApellido()} , ${tur.getPaciente().getNombre()}"></c:out>
										</td>
										<td><a href="#" class="btn btn-success btn-sm"
											idTurno="${tur.getId_turno()}"
											profesional="${tur.getHorario().getProfesional().getApellido()},${tur.getHorario().getProfesional().getNombre()}"
											descPractica="${tur.getHorario().getPractica().getDescripcion()}"
											datosTurno="${tur.getFecha_t()} ${tur.getHora_t()}"
											data-bs-toggle="modal" data-bs-target="#cancelaTurno">Cancelar</a>
											<!--             						REEMPLAZAR ESTE BOTON POR ICONO --></td>
									</tr>
								</c:forEach>
							</c:if>
						</tbody>
