...
List<Prescripcion> presc =  new ArrayList<>();
		...
		try
		{
			...
			while(rs!=null && rs.next())
			{
				...
				pc.setNro_afiliado(rs.getString("pc.nro_afiliado"));
				pc.setNombre(rs.getString("us.nombre"));
				pc.setApellido(rs.getString("us.apellido"));
				
				os.setId_obra_social(rs.getInt("os.id_obra_social"));
				os.setNombre(rs.getString("os.nombre_os"));
				
				pc.setObra_social(os);
			    pres.setPractica(pr);;
				pres.setPaciente(pc);
				presc.add(pres);
